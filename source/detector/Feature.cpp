...
Feature::Feature(int row, int col, double sigma, double theta): m_row(row), m_col(col), m_sigma(sigma), m_theta(theta), m_octave(0), m_level(0)
{}

Feature::Feature(int row, int col, double sigma, double theta, int octave, int level) : m_row(row), m_col(col), m_sigma(sigma), m_theta(theta), m_octave(octave), m_level(level)
{}

int const& Feature::getRow() const
{
  return m_row;
}

int const& Feature::getCol() const
{
  return m_col;
}
