...
void Sift::findExtremaAux(LevelPyramid const& level1, LevelPyramid const& level2, LevelPyramid const& level3)
 {
...
Mat Sift::computeHessian(int row, int col, Mat const& im1, Mat const& im2, Mat const& im3) const
{
...
Mat Sift::computeGradian(int row, int col, cv::Mat const& im1, cv::Mat const& im2, cv::Mat const& im3) const
{
  REQUIRE(im1.rows >= 3 && im1.cols >= 3, "Size of images must be superior or equal at 3");
  REQUIRE(im1.size() == im2.size() && im2.size() == im3.size(), "Dimension of images must be equal");
  REQUIRE(row > 0 && row < im1.rows, "Row don't be in the border");
...
bool Sift::isSupHalfOfStep(cv::Mat delta) const
{
...
cv::Mat Sift::delataConvertToImageFrame(cv::Mat delta) const
{
  REQUIRE(delta.rows == 3 && delta.cols == 1, "Delta must have the size 3 * 1 ");
