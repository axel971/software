...
void DOGPyramid::build()
{
  ...
  for(int i = 0; i < m_octave; ++i)
    for(int j = 0; j < m_level + 1; ++j)
      {
	m_gaussianPyramid.getImage(i, j + 1).convertTo(im1, CV_64FC1);
	m_gaussianPyramid.getImage(i, j).convertTo(im2, CV_64FC1);
	m_data[i * (m_level + 1) + j] = GaussianLevelPyramid(im1 - im2, i, j, m_gaussianPyramid.getSigma(i, j));
      }
